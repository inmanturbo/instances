...
{
    public $signature = 'instances:migrate 
        {--force : Run the command without asking for confirmation} 
        {--fresh : Drop all event-model tables and re-run migrations} 
        {--database= : The database connection to use} 
        {--reset : Rollback all event-model migrations} 
        {--log-only : Log the migration in the database without running it} 
        {--wipe : Drop all event-model tables and delete all migrations}
    ...
    public $description = 'Run the event-model migrations';
    ...
    public function handle(): int
    {
        ...
        if ($this->option('database')) {
            app(DatabaseManager::class)
                ->usingConnection($this->option('database'), fn () => $this->main());
